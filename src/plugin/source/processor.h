...
public:
    ...
    // A named plug-in from the catalog: same processor, but it knows which
    // script it is and does not consult MPVST_SCRIPT_PATH.
    explicit Processor (const CatalogEntry& entry);

    static Steinberg::FUnknown* createInstance (void*);
    static Steinberg::FUnknown* createEffectInstance (void*);
    // The factory hands the catalog entry through as the class context, so
    // one create function serves every generated class.
    static Steinberg::FUnknown* createFromCatalog (void* context);
