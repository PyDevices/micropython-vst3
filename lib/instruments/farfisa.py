...
def handle_event(event_type, channel, note_id, data0, value0, value1, sample_position):
    ...
    if event_type == vstaudio.EVENT_NOTE_ON and value0 > 0.0:
        ...
        # Multi-tone booster creates an aggressive high pass filter sound
        cutoff = 5000.0 if multi_tone > 0.5 else 15000.0
        filter_mode = synthio.FilterMode.HIGH_PASS if multi_tone > 0.5 else synthio.FilterMode.LOW_PASS
        ...
        notes = []
        if bass > 0.01: notes.append(synthio.Note(hz * 0.5, waveform=WAVE_STR, envelope=env, filter=lp, amplitude=amp * bass * 0.2, bend=vib_lfo))
        if strings > 0.01: notes.append(synthio.Note(hz, waveform=WAVE_STR, envelope=env, filter=lp, amplitude=amp * strings * 0.2, bend=vib_lfo))
        if flute > 0.01: notes.append(synthio.Note(hz, waveform=WAVE_FLUTE, envelope=env, filter=lp, amplitude=amp * flute * 0.2, bend=vib_lfo))
        if oboe > 0.01: notes.append(synthio.Note(hz, waveform=WAVE_OBOE, envelope=env, filter=lp, amplitude=amp * oboe * 0.2, bend=vib_lfo))
        if trumpet > 0.01: notes.append(synthio.Note(hz, waveform=WAVE_TRUMPET, envelope=env, filter=lp, amplitude=amp * trumpet * 0.2, bend=vib_lfo))
