...
archive="$dist_dir/MicroPythonVST3-0.1.0-windows-x86_64.zip"
...
cp "$repo_dir/docs/windows-workflow.md" "$stage_dir/MicroPythonVST3-0.1.0/"
cp -a "$repo_dir/examples" "$stage_dir/MicroPythonVST3-0.1.0/"

mkdir -p "$dist_dir"
rm -f -- "$archive"
(cd "$stage_dir" && cmake -E tar cf "$archive" --format=zip MicroPythonVST3-0.1.0)
